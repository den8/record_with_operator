    ...
    module ClassMethods
      ...
      def records_with_operator_on(*args)
        ...
        if self.table_exists?
          belongs_to :creater_user, {:foreign_key => "created_by", :class_name => RecordWithOperator.config[:operator_class_name]}.merge(RecordWithOperator.config[:operator_association_options]) if records_creator?
